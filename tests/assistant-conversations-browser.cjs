...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{for(const width of [390,1366]){
 ...
 await page.route('**/__conversations',async route=>{const {args}=route.request().postDataJSON(),p=args.p_payload;assert.equal(p.employee,'TEST');let data=[];
  ...
  if(args.p_action==='append'){assert.ok(messages.has(p.conversationId));data={id:generations,message:p.message,reply:p.reply,created_at:new Date().toISOString()};messages.get(p.conversationId).push(data);if(messages.get(p.conversationId).length===1)conversations.find(c=>c.id===p.conversationId).title=p.message;}
  if(args.p_action==='rename_conversation'){data=conversations.find(c=>c.id===p.conversationId);data.title=p.title;}
 ...
 async function create(title){await page.getByRole('button',{name:'Nova conversa',exact:true}).click();assert.equal(await page.getByLabel('Título da conversa',{exact:true}).count(),0);await page.getByLabel('Mensagem ao assistente').waitFor();}
 ...
 await page.getByRole('button',{name:'Editar título de Tema A',exact:true}).click();await page.getByLabel('Título da conversa',{exact:true}).fill('Relato CHT');await page.getByRole('button',{name:'Salvar título',exact:true}).click();

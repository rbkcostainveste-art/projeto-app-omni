const {test}=require('node:test');const assert=require('node:assert/strict');const fs=require('node:fs');const ts=require('typescript');const lib={};new Function('exports',ts.transpileModule(fs.readFileSync('src/lib/coordination-day.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText)(lib);
test('coordination uses Brazilian day, including after UTC midnight',()=>{assert.equal(lib.operationalDay(new Date('2026-09-07T02:59:59Z')),'2026-09-06');assert.equal(lib.operationalDay(new Date('2026-09-07T03:00:00Z')),'2026-09-07');});

...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true,args:['--use-fake-device-for-media-stream','--use-fake-ui-for-media-stream']});try{for(const width of [390,1366]){
 ...
 fail=false;await page.getByRole('button',{name:'Enviar à IA',exact:true}).click();await page.getByText('Campos preenchidos. Confira no formulário antes de salvar.',{exact:true}).waitFor();assert.equal(transcriptions,2);

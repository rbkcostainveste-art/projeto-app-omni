...
begin
 ...
 v:=public.get_flight_operation(fid);if v->>'nextFlightId'<>fid2 then raise exception 'Next flight missing';end if; denied:=false;begin perform public.record_flight_operation(fid,gen_random_uuid(),20,'approve','{"key":"postflight","result":"ok"}');exception when others then denied:=sqlerrm like '%Assine Entre voos%';end;if not denied then raise exception 'Duplicate postflight allowed';end if;
 ...
 perform set_config('request.jwt.claim.sub',admin_id::text,true);perform public.mutate_shared_item('flights',fid2,'{"cancelled":true}','update'); perform set_config('request.jwt.claim.sub',mech_id::text,true);v:=public.get_flight_operation(fid);if v->>'nextFlightId' is not null then raise exception 'Cancelled flight blocks final inspection';end if;v:=public.record_flight_operation(fid,gen_random_uuid(),20,'approve','{"key":"postflight","result":"ok"}');if v#>>'{checks,postflight,targetFlightId}'<>fid then raise exception 'Final inspection missing';end if; perform set_config('request.jwt.claim.sub',admin_id::text,true);perform public.mutate_shared_item('flights',fid2,'{"commander":"not-assigned"}','update');

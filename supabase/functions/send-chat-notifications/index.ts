 ...
 for(const job of jobs||[]){
  ...
  if(msg&&person?.active&&member&&!receipt?.read_at)for(const target of targets||[]){
   try{
    ...
    sent++;
   }catch(e){const status=(e as {statusCode?:number}).statusCode;if(status===404||status===410)await db.from("push_subscriptions").delete().eq("id",target.id);else retry=true;}
  }
  await db.from("chat_push_jobs").update({done:!retry}).eq("message_id",job.message_id).eq("employee_number",job.employee_number);
 }

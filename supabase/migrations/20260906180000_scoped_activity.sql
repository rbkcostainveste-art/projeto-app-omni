...
declare actor public.device_identities; role_name text; origin public.maintenance_records; targets jsonb; plane jsonb; actions jsonb:='[]'; action_id text; payload jsonb; stamp text:=to_char(now() at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
begin
 ...
 insert into public.operational_wall_posts(id,base,audience_area,pinned,essential,resolved,data) values(p_id,p_base,'maintenance',false,false,false,payload);
 if origin.id is not null then update public.maintenance_records set data=jsonb_set(data,'{entries}',coalesce(data->'entries','[]')||jsonb_build_array(jsonb_build_object('id',gen_random_uuid(),'kind','assignment','description','Gerou ação pendente: '||p_category||' — '||trim(p_title),'employeeNumber',actor.employee_number,'at',stamp,'wallPostId',p_id,'assignedTo',to_jsonb(p_assigned)))),revision=revision+1,updated_at=now() where id=origin.id;end if;
 if p_category in('Giro em baixa','Giro em alta','Voo de vibração','Voo de manutenção') then
 for plane in select value from jsonb_array_elements(actions) loop perform public.configure_maintenance_operation(p_id,coalesce(p_plan,'{}')||jsonb_build_object('flightId','maintenance-'||(plane->>'id')));end loop;
 end if;return p_id;
end $$;

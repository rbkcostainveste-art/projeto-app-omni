alter table public.personal_notes add column notify boolean not null default false;
...
create or replace function public.claim_note_reminders() returns setof public.personal_notes language sql security definer set search_path='' as $$
 update public.personal_notes set reminder_attempts=reminder_attempts+1,reminder_available_at=now()+interval '2 minutes' where id in(select id from public.personal_notes where notify and remind_at<=now() and not reminder_done and reminder_attempts<5 and reminder_available_at<=now() order by reminder_available_at for update skip locked limit 50) returning *
$$;

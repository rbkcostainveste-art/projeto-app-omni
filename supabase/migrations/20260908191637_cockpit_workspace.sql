-- Optional operational preparation. The external eDB remains the official record.
create table public.cockpit_entries (
 id text primary key, kind text not null check(kind in ('qualification','person','location','contract','settings','preparation','document','diary','counter','occurrence','duty')),
 subject text references public.authorized_users(employee_number), flight_id text,
 data jsonb not null default '{}' check(jsonb_typeof(data)='object'),
 revision integer not null default 1, created_by text not null, updated_by text not null,
 created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create index cockpit_entries_person_idx on public.cockpit_entries(subject,kind);
create index cockpit_entries_flight_idx on public.cockpit_entries(flight_id,kind);
create unique index cockpit_preparation_unique on public.cockpit_entries(flight_id) where kind='preparation';
create unique index cockpit_person_unique on public.cockpit_entries(subject) where kind='person';
create unique index cockpit_duty_day_unique on public.cockpit_entries(subject,(data->>'date')) where kind='duty';
create table public.cockpit_audit(id bigint generated always as identity primary key,entry_id text not null references public.cockpit_entries(id),revision integer not null,actor text not null,at timestamptz not null default now(),data jsonb not null);
alter table public.cockpit_entries enable row level security;
alter table public.cockpit_audit enable row level security;
revoke all on public.cockpit_entries,public.cockpit_audit from public,anon,authenticated;

create function private.cockpit_access(k text,s text,fid text,creator text,writing boolean default false) returns boolean language plpgsql stable security definer set search_path='' as $$
declare actor text; role_name text; admin boolean; f jsonb;
begin
 select u.employee_number,u.access_profile,(d.is_admin or u.access_profile in ('admin','app_manager')) into actor,role_name,admin from public.device_identities d join public.authorized_users u using(employee_number) where d.auth_user_id=auth.uid() and u.active;
...
create function public.cockpit(p_action text,p_payload jsonb default '{}') returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if k='person' then
  if exists(select 1 from public.authorized_users where employee_number=subject_id and access_profile in ('toolroom','coordination','dispatch')) then body:=body||jsonb_build_object('regime','Mensalista');end if;
 end if;

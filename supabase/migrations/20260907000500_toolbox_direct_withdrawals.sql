create or replace function public.toolbox_command_before_partial(p_action text,p_payload jsonb default '{}') returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if p_action in('take_tool','correct_tool_selection') then
  ...
  if p_action='correct_tool_selection' then
   ...
   update public.toolbox_events set tool_refs=snapshot,description=names,status='open',approved_by=null,approved_at=null where id=ev.id;
  ...
  update public.toolbox_events set tool_refs=snapshot,status='open',approved_by=null,approved_at=null where id=(result->>'id')::uuid;
...
-- The borrower's original withdrawal remains the signature. Do not forge an owner's approval.
insert into public.toolbox_audit(action,target_id,actor,payload)
select 'withdrawal_confirmation_removed',id,'SYSTEM',jsonb_build_object('reason','Retirada direta conforme nova regra operacional','originalBorrower',employee_number,'originalSignedAt',created_at,'previousStatus',status)
from public.toolbox_events where status='awaiting_approval';
update public.toolbox_events set status='open',approved_by=null,approved_at=null where status='awaiting_approval';

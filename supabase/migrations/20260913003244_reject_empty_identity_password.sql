-- A NULL crypt() result must never bypass the password comparison.
-- Preserve the existing identity claim behavior for valid credentials.
...
declare v_user public.authorized_users; v_uid uuid:=auth.uid(); begin
 ...
 if nullif(p_password,'') is null then raise exception 'Matrícula ou senha inválida'; end if;

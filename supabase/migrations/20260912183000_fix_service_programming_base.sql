...
begin
  ...
  if not global_access and nullif(trim(person.assigned_base),'') is null then
    raise exception 'Defina sua base antes de programar serviços';
  end if;
  ...
  for item in select value from jsonb_array_elements(p_items) order by value->>'id' loop
    ...
    if jsonb_typeof(assigned)<>'array' then raise exception 'Lista de executantes inválida';end if;
    ...
    if jsonb_typeof(attachments)<>'array' then raise exception 'Lista de anexos inválida';end if;
    ...
    if global_access then
      base_value:=coalesce(nullif(trim(item->>'base'),''),nullif(trim(aircraft_item->>'base'),''));
    else
      base_value:=trim(person.assigned_base);
    end if;
    if base_value is null or not exists(
      select 1
      from public.shared_app_state state
      ...
      raise exception 'Base operacional inválida';
    end if;
    ...
    if exists(
      ...
      where not exists(
        ...
        where candidate.employee_number=employee
          ...
          and candidate.assigned_base=base_value
      ...
      raise exception 'Selecione executantes ativos da base desta passagem';
    end if;
    ...
    insert into public.maintenance_records(
      ...
      record_id,'inspection',base_value,model_value,prefix_value,
...
set base=person.assigned_base
from public.authorized_users person
where record.record_type='inspection'
  and record.created_by=person.employee_number
  and person.employee_number='0200'
  and person.assigned_base='Jacarepaguá'
  and record.base='Cabo Frio'
  and record.created_at>=timestamptz '2026-09-12 16:50:00+00'
  and record.created_at<timestamptz '2026-09-12 17:00:00+00';

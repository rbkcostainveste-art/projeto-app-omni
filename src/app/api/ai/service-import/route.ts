...
import {serviceImportSchema,parseServiceImportRequest,parseServiceImportExtraction,serviceImportInstructions} from "@/lib/service-import";
...
export async function POST(request:Request){
  ...
  try{
    ...
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},signal:AbortSignal.any([request.signal,AbortSignal.timeout(110000)]),body:JSON.stringify({model:process.env.OPENAI_MODEL||"gpt-5.4-mini",store:false,max_output_tokens:14000,instructions:`${serviceImportInstructions}\nCatálogo autorizado de aeronaves: ${JSON.stringify(fleet)}. Use somente prefixos presentes nesse catálogo. Explique no reply qualquer página parcial, trecho ilegível ou item sem WO Task.`,input:[{role:"user",content}],text:{format:{type:"json_schema",name:"service_import",strict:true,schema:serviceImportSchema}}})});
    ...
    return json(parseServiceImportExtraction(JSON.parse(output)));

...
import {queryNotes} from './assistant-notes';
...
export type QueryArgs={dataset:'timeline'|'notices'|'assignments'|'maintenance'|'drying'|'washing'|'fleet'|'flights'|'passage'|'tools'|'cockpit'|'notes';query:string|null;prefix:string|null;base:string|null;from:string|null;until:string|null;status:'open'|'closed'|'all';mine:boolean;offset:number;id:string|null};
...
export function validateQuery(value:unknown):QueryArgs {
 ...
 if(!['timeline','notices','assignments','maintenance','drying','washing','fleet','flights','passage','tools','cockpit','notes'].includes(q.dataset)||!['open','closed','all'].includes(q.status)||typeof q.mine!=='boolean'||!Number.isInteger(q.offset)||q.offset<0||q.offset>500)throw Error('Consulta inválida.');
 ...
 if(q.id&&['maintenance','drying','tools','notes'].includes(q.dataset)&&!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(q.id))throw Error('id deve ser o UUID interno do card, nunca código PAN ou prefixo. Para buscar CHT, use prefix=CHT e id=null.');
...
export async function assistantQuery(client:SupabaseClient,actor:AssistantActor,q:QueryArgs,signal:AbortSignal,timeZone='America/Sao_Paulo'):Promise<QueryResult>{
 ...
 try {
  if(q.dataset==='notes')return await queryNotes(client,actor,q,signal,timeZone);

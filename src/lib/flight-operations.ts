...
export type FlightCounter = {key:string;label:string;unit:string;value:number;inProgress:boolean};
export type MaintenanceCheck = { kind: string; targetFlightId: string; execution?: { actor: string; at: string }; approval?: { actor: string; at: string; result: 'ok'|'no'; invalidated?:boolean; reason?:string } };
export type OperationData = { preparation?:import("./preparation-display").PreparationState; counterSource?:string; counters?:FlightCounter[]; events: OperationEvent[]; checks: Record<string, MaintenanceCheck>; revision: number; first: boolean; previousFlightId: string|null; nextFlightId: string|null; day: string; canPilot: boolean; canSign: boolean; canExecute: boolean; closed: boolean };

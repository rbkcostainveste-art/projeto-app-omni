...
export function assistantMediaContent(attachments:AssistantAttachment[],maxBytes=2000000){
 let total=0;
 return attachments.map(({name,data})=>{
  ...
  if(!valid||total>maxBytes)throw Error('Anexo inválido ou acima de 2 MB.');

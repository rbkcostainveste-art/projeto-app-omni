...
export function pendingMaintenanceOnDay(item:{date?:string;createdAt?:string;planningStatus?:string;status?:string},day:string){
 if(['completed','ok','resolved','closed'].includes(item.status||''))return false;
 const value=item.date||item.createdAt;
 const date=value&&/^\d{4}-\d{2}-\d{2}$/.test(value)?value:value&&Number.isFinite(new Date(value).getTime())?operationalDay(new Date(value)):'';
 // Unscheduled requests and overdue work stay in the coordinator's queue.
 return item.planningStatus==='planned'||!!date&&date<=day;
}

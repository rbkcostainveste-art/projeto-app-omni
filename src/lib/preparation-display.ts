...
export type PreparationState={status:'ready'|'reconfirm'|'pending'|'inactive';canConfirm:boolean;pending:string[];blocked:boolean;actor?:string;at?:string;reason?:string;fingerprint?:string;checklist?:{total:number;approved:number;first:boolean};blockers?:PreparationBlocker[];blockerCount?:number};

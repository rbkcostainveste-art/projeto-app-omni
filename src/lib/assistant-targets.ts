export type AssistantTargetRef={kind:'maintenance'|'drying'|'wall'|'activity'|'flight'|'passage'|'tool';id:string};
...
export function parseTarget(value:unknown):AssistantTargetRef|null{
 ...
 return (item.kind==='maintenance'||item.kind==='drying'||item.kind==='wall'||item.kind==='activity'||item.kind==='flight'||item.kind==='passage'||item.kind==='tool')&&typeof item.id==='string'&&((item.kind==='wall'||item.kind==='activity'||item.kind==='flight'||item.kind==='passage'||item.kind==='tool')?/^[a-zA-Z0-9_-]{1,160}$/.test(item.id):uuid.test(item.id))?{kind:item.kind,id:(item.kind==='wall'||item.kind==='activity'||item.kind==='flight'||item.kind==='passage'||item.kind==='tool')?item.id:item.id.toLowerCase()}:null;
}
...
export function splitTargetLinks(reply:string){
 ...
 const text=reply.replace(/^\[([^\]\n]{1,220})\]\(flight-ia:\/\/(maintenance|drying|wall|activity|flight|passage|tool)\/([a-zA-Z0-9_-]{1,160})\)\s*$/gim,(whole,label,kind,id)=>{const ref=parseTarget({kind:kind.toLowerCase(),id});if(!ref)return whole;if(cards.length<12)cards.push({...ref,label});return '';}).trim();
 return {text,cards};
}

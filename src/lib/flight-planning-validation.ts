...
export function confirmationMissing(flight:Plan){
 return [!validDate(flight.date)&&'data',!validClock(flight.departure)&&'horário de saída',!flight.destination?.trim()&&'destino',!(typeof flight.duration==='number'?flight.duration>0:validClock(flight.duration)&&flight.duration!=='00:00')&&'duração'].filter(Boolean) as string[];
}

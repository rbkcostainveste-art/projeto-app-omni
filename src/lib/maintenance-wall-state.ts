...
export function timelineStatus(post: WallPost) {
  ...
  if(post.actions.length?post.actions.every(a=>a.status==="resolved"):post.resolved) return "Concluído";
...
export function timelineTone(post: WallPost, user: string) {
  ...
  if((!post.actions.length&&post.maintenanceResult === "satisfactory") || (!post.actions.length&&post.resolved) || post.actions.length > 0 && post.actions.every((action) => action.status === "satisfactory" || action.status === "resolved")) return "border-emerald-500";

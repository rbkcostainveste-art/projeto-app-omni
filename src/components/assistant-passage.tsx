...
export function PassageAssistant({item,labels,client,user,readOnly,onChange,requireSignature}:{item:Passage;labels:Partial<Record<PassageCheckKey,string>>;client:SupabaseClient|null;user:string;readOnly:boolean;onChange:(id:string,change:(item:Passage)=>Passage)=>void|boolean|Promise<void|boolean>;requireSignature:(action:()=>void|Promise<void>,label?:string)=>Promise<boolean>}){
 ...
 async function apply(values:Record<string,string>){
  ...
  const outcome:{saved?:boolean}={};
  const ok=await requireSignature(async()=>{const current=latest.current;if(current.readOnly||current.item.revision!==snapshot.revision||current.item.updatedAt!==snapshot.updatedAt)throw Error('A passagem mudou. Revise o pedido novamente.');const result=await onChange(snapshot.id,record=>passageAssistantPatch(record,labels,values,user,new Date().toISOString()).next);outcome.saved=typeof result==='boolean'?result:undefined;},'Confirmar alterações da passagem');
  if(!ok)throw Error('Alteração não confirmada.');
  if(outcome.saved===false)throw Error('A passagem não foi salva. Confira a conexão e tente novamente.');
  return outcome.saved===true?{status:'saved' as const,message:'Alterações salvas na passagem de pista.'}:{status:'pending' as const,message:'Campos atualizados na passagem. A sincronização ainda não foi confirmada.'};
 }

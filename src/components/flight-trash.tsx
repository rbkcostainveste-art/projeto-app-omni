...
export function FlightTrash({flights,onClose,onRestore,onDelete}:{flights:TrashedFlight[];onClose:()=>void;onRestore:(id:string)=>void;onDelete:(id:string)=>Promise<boolean>}){
 ...
 return <ModalLayer><div className="fixed inset-0 z-50 grid place-items-center bg-[#071a30]/60 p-2 backdrop-blur-sm" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)onClose();}}><section role="dialog" aria-modal="true" aria-labelledby="flight-trash-title" className="flex max-h-[90dvh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">

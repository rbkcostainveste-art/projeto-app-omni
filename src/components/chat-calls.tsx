...
import {Mic,MicOff,Video,VideoOff,Phone,PhoneOff,MonitorUp,X} from "lucide-react";
...
function CallRoom({active,client,onClose}:{active:Active;client:SupabaseClient;onClose:()=>void}){
 ...
 const finish=useCallback(()=>{if(ending.current)return;ending.current=true;release();void callRpc(client,'leave',{id,session}).catch(()=>{});leave.current();},[client,id,session,release]);
 useEffect(()=>{
 ...
 async function share(){if(screen.current){await stopSharing();return;}setControlBusy(true);try{const media=await navigator.mediaDevices.getDisplayMedia({video:true,audio:false});if(!mounted.current||ending.current){media.getTracks().forEach(t=>t.stop());return;}screen.current=media;await replaceVideo(media.getVideoTracks()[0]);media.getVideoTracks()[0].onended=()=>void stopSharing().catch(e=>setError(e.message));setSharing(true);setLocal(media);}catch(e){setError((e as Error).name==='NotAllowedError'?'Compartilhamento cancelado.':(e as Error).message);}finally{setControlBusy(false);}}
 const connected=Object.values(connections).filter(s=>s==='connected').length;
 return <ModalLayer><section className="m-auto flex max-h-[94dvh] w-[min(96vw,1000px)] flex-col overflow-hidden rounded-2xl bg-slate-950 text-white shadow-xl"><header className="flex items-center justify-between p-4"><div><h2 className="font-bold">{ended?'Chamada encerrada':connected?'Em chamada':'Chamando / conectando…'}</h2><p className="text-xs text-slate-300">{members.filter(m=>m.state==='joined').length} participantes · até 6 pessoas</p></div><button aria-label="Encerrar chamada" onClick={finish} className="rounded-full bg-red-700 p-3"><PhoneOff size={20}/></button></header><div className="grid min-h-0 flex-1 gap-3 overflow-auto p-3 sm:grid-cols-2"><StreamView stream={local} name={sharing?'Sua tela':'Você'} muted/>{Object.entries(remote).map(([key,media])=><StreamView key={key} stream={media} name={members.find(m=>m.session===key)?.name||'Colega'}/>)}{!connected&&!ended?<p role="status" className="p-4 text-sm">Aguardando os colegas atenderem e a conexão de áudio/vídeo.</p>:null}</div>{error?<p role="alert" className="px-4 py-2 text-sm text-amber-200">{error}</p>:null}<footer className="flex flex-wrap justify-center gap-3 p-4">{ended?<button onClick={finish} className="rounded-xl bg-slate-700 px-4 py-2">Fechar</button>:<><button aria-label={muted?'Ativar microfone':'Silenciar microfone'} className="rounded-full bg-slate-700 p-3" onClick={()=>{stream.getAudioTracks().forEach(t=>t.enabled=muted);setMuted(!muted);}}>{muted?<MicOff/>:<Mic/>}</button><button disabled={controlBusy} aria-label={cameraOn?'Desligar câmera':'Ligar câmera'} className="rounded-full bg-slate-700 p-3" onClick={()=>void toggleCamera()}>{cameraOn?<Video/>:<VideoOff/>}</button><button disabled={controlBusy||!navigator.mediaDevices?.getDisplayMedia} aria-label={sharing?'Parar compartilhamento':'Compartilhar tela'} className="rounded-full bg-slate-700 p-3 disabled:opacity-30" onClick={()=>void share()}><MonitorUp/></button><button onClick={finish} className="inline-flex items-center gap-2 rounded-full bg-red-700 px-5 py-3"><X size={18}/>Encerrar</button></>}</footer></section></ModalLayer>;
}

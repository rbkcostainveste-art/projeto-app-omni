...
import {Mic,MicOff,Video,VideoOff,Phone,PhoneOff,MonitorUp,SwitchCamera,X,Maximize,Minimize,LayoutGrid,PanelTop} from "lucide-react";
import styles from "./call-layout.module.css";
...
function StreamView({stream,name,muted=false,audioOnly=false}:{stream:MediaStream;name:string;muted?:boolean;audioOnly?:boolean}){
 ...
 return <div className="relative h-full min-h-0 overflow-hidden rounded-xl bg-slate-800"><video aria-label={name} ref={ref} autoPlay playsInline muted={muted} onPlaying={()=>setBlocked(false)} className="h-full min-h-0 w-full object-contain"/><span className="absolute bottom-2 left-2 rounded bg-black/60 px-2 py-1 text-xs text-white">{name}</span>{blocked?<button className="absolute inset-0 text-white" onClick={()=>void ref.current?.play().then(()=>setBlocked(false)).catch(()=>{})}>Toque para ouvir e ver</button>:null}</div>;
}
...
function CallRoom({active,client,onClose,onRecorded}:{active:Active;client:SupabaseClient;onClose:()=>void;onRecorded:(file:File)=>void}){
 ...
 const [layout,setLayout]=useState<'focus'|'grid'>('focus'),[focused,setFocused]=useState(''),[expanded,setExpanded]=useState(false);
 const ownsFullscreen=useRef(false);
 useEffect(()=>{
  const changed=()=>{if(ownsFullscreen.current&&!document.fullscreenElement){ownsFullscreen.current=false;setExpanded(false);}};
  document.addEventListener('fullscreenchange',changed);
  return()=>{document.removeEventListener('fullscreenchange',changed);if(ownsFullscreen.current&&document.fullscreenElement)void document.exitFullscreen().catch(()=>{});};
 },[]);
 async function toggleExpanded(){
  if(expanded){setExpanded(false);if(ownsFullscreen.current&&document.fullscreenElement)await document.exitFullscreen().catch(()=>{});ownsFullscreen.current=false;return;}
  setExpanded(true);
  if(!document.fullscreenElement&&document.documentElement.requestFullscreen){try{await document.documentElement.requestFullscreen();ownsFullscreen.current=true;}catch{/* Keep the viewport layout when native fullscreen is unavailable. */}}
 }
 ...
 const tiles=[{key:'local',media:local,name:sharing?'Sua tela':'Você',muted:true},...Object.entries(remote).map(([key,media])=>({key,media,name:members.find(m=>m.session===key)?.name||'Colega',muted:false}))];
 const mainKey=tiles.some(tile=>tile.key===focused)?focused:tiles.find(tile=>!tile.muted)?.key||'local';
 return <ModalLayer><section aria-label="Área da chamada" className={`${videoCall?styles.room:'m-auto flex max-h-[94dvh] w-[min(96vw,1000px)] flex-col overflow-hidden rounded-2xl'} ${expanded?styles.expanded:''} bg-slate-950 text-white shadow-xl`}><header className="flex items-center justify-between p-4"><div><h2 className="font-bold">{ended?'Chamada encerrada':connected?'Em chamada':'Chamando / conectando…'}</h2><p className="text-xs text-slate-300">{members.filter(m=>m.state==='joined').length} participantes · até 6 pessoas</p></div><button aria-label={ended?"Fechar chamada":"Encerrar chamada"} onClick={ended?onClose:finish} className={`rounded-full bg-red-700 p-3 ${!videoCall&&!ended?"hidden":""}`}><PhoneOff size={20}/></button></header>{videoCall?<>
 <div className="flex shrink-0 flex-wrap gap-2 px-3 pb-2" role="group" aria-label="Visualização da chamada">
  <button aria-pressed={layout==='focus'} onClick={()=>setLayout('focus')} className="inline-flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2 text-sm"><PanelTop size={16}/>Destaque</button>
  <button aria-pressed={layout==='grid'} onClick={()=>setLayout('grid')} className="inline-flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2 text-sm"><LayoutGrid size={16}/>Grade</button>
  <button aria-label={expanded?'Restaurar chamada':'Tela cheia'} onClick={()=>void toggleExpanded()} className="ml-auto inline-flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2 text-sm">{expanded?<Minimize size={16}/>:<Maximize size={16}/>}<span>{expanded?'Restaurar':'Tela cheia'}</span></button>
 </div>
 <div className={`${styles.stage} ${layout==='grid'?styles.grid:styles.focus} ${tiles.length<=2?styles.pair:''}`} data-layout={layout} style={{'--rows':Math.ceil(tiles.length/2),'--wide-rows':Math.ceil(tiles.length/3),'--peers':Math.max(1,tiles.length-1)} as React.CSSProperties}>
  {tiles.map((tile,index)=><div key={tile.key} data-main={tile.key===mainKey} className={`${styles.tile} ${tile.key===mainKey?styles.main:styles.thumbnail}`} style={{'--slot':tiles.slice(0,index).filter(t=>t.key!==mainKey).length+1} as React.CSSProperties}>
   <StreamView stream={tile.media} name={tile.name} muted={tile.muted}/>
   <button aria-label={`Destacar ${tile.name}`} aria-pressed={layout==='focus'&&tile.key===mainKey} onClick={()=>{setFocused(tile.key);setLayout('focus');}} className={styles.spotlight}><Maximize size={16}/></button>
  </div>)}
 </div>
 </>:<div className="grid min-h-0 flex-1 gap-3 overflow-auto p-3 sm:grid-cols-2">{Object.entries(remote).map(([key,media])=><StreamView key={key} audioOnly stream={media} name={members.find(m=>m.session===key)?.name||'Colega'}/>)}</div>}
 {!connected&&!ended?<p role="status" className="shrink-0 px-4 py-2 text-sm">Aguardando os colegas atenderem e a conexão de áudio/vídeo.</p>:null}{error?<p role="alert" className="px-4 py-2 text-sm text-amber-200">{error}</p>:null}<footer className="flex shrink-0 flex-wrap justify-center gap-2 p-2 sm:gap-3 sm:p-4">{ended?<button onClick={onClose} className="rounded-xl bg-slate-700 px-4 py-2">Fechar</button>:<><button aria-label={muted?'Ativar microfone':'Silenciar microfone'} className="rounded-full bg-slate-700 p-3" onClick={()=>{stream.getAudioTracks().forEach(t=>t.enabled=muted);setMuted(!muted);}}>{muted?<MicOff/>:<Mic/>}</button>{videoCall?<><button disabled={controlBusy} aria-label={cameraOn?'Desligar câmera':'Ligar câmera'} className="rounded-full bg-slate-700 p-3" onClick={()=>void toggleCamera()}>{cameraOn?<Video/>:<VideoOff/>}</button><button disabled={controlBusy} aria-label={sharing?'Parar compartilhamento':'Compartilhar tela'} className="rounded-full bg-slate-700 p-3 disabled:opacity-30" onClick={()=>void share()}><MonitorUp/></button><button disabled={controlBusy||sharing} aria-label="Trocar câmera" onClick={()=>void flipCamera()} className="rounded-full bg-slate-700 p-3"><SwitchCamera/></button></>:null}<button aria-label={videoCall?"Encerrar":"Encerrar chamada"} onClick={finish} className="inline-flex items-center gap-2 rounded-full bg-red-700 px-5 py-3"><X size={18}/>Encerrar</button></>}</footer>{Object.entries(recorders).some(([key,value])=>value&&members.some(m=>m.session===key&&m.state==='joined'))?<p role="status" className="bg-red-800 p-2 text-center">● Um participante está gravando esta chamada</p>:null}{videoCall?<div className="p-3"><CallRecording streams={[recordingLocal,...Object.values(remote)]} ended={ended} onState={recordingState} onRecorded={onRecorded}/></div>:null}</section></ModalLayer>;
}

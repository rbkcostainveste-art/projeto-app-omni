...
type Workspace={register:(target:Target)=>()=>void;open:()=>void;close:()=>void};
...
export function AssistantWorkspaceProvider({children}:{children:ReactNode}){
 ...
 const open=useCallback(()=>setOpened(true),[]),close=useCallback(()=>setOpened(false),[]);
 useEffect(()=>{
 ...
 const active=scoped.toSorted((a,b)=>a.priority-b.priority).at(-1);
 ...
 return <Context.Provider value={{register,open,close}}>{children}{host&&root?createPortal(<>

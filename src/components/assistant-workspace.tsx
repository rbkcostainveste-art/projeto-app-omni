...
export function AssistantWorkspaceProvider({children}:{children:ReactNode}){
 ...
 return <Context.Provider value={{register,open,close,screen,setScreen,transfer,beginTransfer,claimTransfer,setNavigator,navigate,navigationAvailable}}>{children}{host&&root?createPortal(<>
  {!opened&&!suspended?<button type="button" aria-label={`Abrir assistente IA · ${label}`} title="Abrir assistente" onClick={()=>open()} className="fixed bottom-24 right-4 z-[250] grid h-11 w-11 place-items-center rounded-full bg-blue-700 text-white shadow-lg hover:bg-blue-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 lg:bottom-3"><Bot size={22} aria-hidden="true"/></button>:null}

...
export function FlightOperations({supabase,flight,showDocumentation=false,readOnly=false,requireSignature}:Props){
 ...
 return <section className="space-y-3"><AircraftTechnicalStatus client={supabase} prefix={flight.prefix}/>{showDocumentation?<div className="flex flex-wrap gap-2"><button type="button" disabled={readOnly} onClick={()=>openCockpit(flight.id,undefined,"new-occurrence")} className="min-h-11 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-800">Registrar ocorrência</button><button type="button" onClick={()=>openCockpit(flight.id,undefined,"edb")} className="min-h-11 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-800">Documentação do voo</button><details><summary className="min-h-11 cursor-pointer px-3 py-2 text-sm">Mais ações</summary><button type="button" className="min-h-11 px-3 text-sm text-blue-800" onClick={()=>openCockpit(flight.id,undefined,"preparation")}>Preparação do voo</button><button type="button" className="min-h-11 px-3 text-sm text-blue-800" onClick={()=>openCockpit(flight.id,undefined,"counter")}>Contadores</button></details></div>:null}<FlightPosition supabase={supabase} flightId={flight.id} readOnly={readOnly} requireSignature={requireSignature}/>
  ...
  {open?<OperationDialog title={`${flight.prefix} · ${flight.model}`} onClose={()=>{if(!busy)setOpen(false);}}>
   ...
   {controls}
   <h4 className="mt-5 font-bold">Registros deste voo</h4><div className="mt-2 space-y-2">{data.events.map(event=><article key={event.id} className="rounded-xl border p-3 text-sm"><strong>{eventLabels[event.type]||event.type}</strong><p className="mt-1 text-xs">{new Date(event.at).toLocaleString('pt-BR')} · mat. {event.actor}</p>{event.corrections?.length?<p className="mt-1 text-xs text-amber-800">Horário corrigido · {event.corrections.length} correção(ões) preservada(s) no histórico</p>:null}{data.canPilot?<><div className="mt-2 flex gap-4"><button disabled={disabled} onClick={()=>{setDeleting(null);setCorrection({id:event.id,at:localInput(event.at)});}} className="min-h-10 text-xs font-bold text-blue-700">Corrigir horário</button><button disabled={disabled} onClick={()=>{setCorrection(null);setDeleting(event.id);}} className="min-h-10 text-xs font-bold text-red-700">Excluir evento</button></div>{deleting===event.id?<div role="group" aria-label="Confirmar exclusão do evento" className="mt-2 rounded-lg border border-red-200 bg-red-50 p-3"><p className="text-sm">Excluir este registro de {eventLabels[event.type]}? Os botões e contadores serão recalculados. A exclusão ficará no histórico.</p>{error?<p role="alert" className="mt-2 text-sm font-semibold text-red-700">{error}</p>:null}<div className="mt-3 flex gap-2"><button disabled={disabled} onClick={()=>void send("delete_event",{id:event.id})} className="min-h-11 rounded-lg bg-red-700 px-3 text-sm font-bold text-white">Confirmar exclusão</button><button disabled={busy} onClick={()=>setDeleting(null)} className="min-h-11 rounded-lg border px-3 text-sm">Voltar</button></div></div>:null}</>:null}</article>)}</div>

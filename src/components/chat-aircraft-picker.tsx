...
export function ChatAircraftPicker({aircraft,value,onChange}:{aircraft:{prefix:string}[];value:string;onChange:(prefix:string)=>void}){
 const id=useId();
 ...
 return <section className="space-y-2">
  <label htmlFor={id} className="block text-sm">Aeronave (opcional)</label>
  ...
  <div className="flex items-center gap-2 text-xs">{value?<><span className="rounded-full bg-blue-50 px-3 py-1 font-bold text-blue-800">{value}</span><button type="button" onClick={()=>select("")} className="text-slate-600 underline">Remover aeronave</button></>:<span className="text-slate-500">Sem aeronave — pode iniciar a conversa assim.</span>}</div>
 </section>;
}

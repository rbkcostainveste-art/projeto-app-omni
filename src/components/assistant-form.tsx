...
export function AssistantForm({form,client,user,onApply,disabled=false}:{form:AssistantFormContext;client:SupabaseClient|null;user:string;onApply:(values:Record<string,string>)=>void|Promise<void>;disabled?:boolean}){
 ...
 return <><AssistantTarget id={`form:${form.id}`} label={form.label} priority={5} revision={JSON.stringify([form,disabled])} content={content}/><button type="button" disabled={disabled} onClick={()=>workspace?.open(`form:${form.id}`)} className="mb-3 min-h-11 rounded-xl border border-blue-200 bg-blue-50 px-3 text-sm font-semibold text-blue-700">Preencher ou conversar com IA</button></>;
}

...
| Relatos | Relatos reais abertos/fechados por prefixo/base | Novo: prefixo, título, descrição, TC; existente: correção, execução, comentário e geração de ação | Evidências técnicas estruturadas publicadas (componente, ação, testes, medição, documento e justificativa); nenhuma liberação inferida |
